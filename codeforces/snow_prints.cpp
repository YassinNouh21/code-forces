...
int main() {
    ...
    char l = 'L';
    char r = 'R';
    bool caseL = count(arr, arr + t, r) == 0 ? true : false;
    bool caseR = count(arr, arr + t, l) == 0 ? true : false;
    int start = 0;
    int end = 0;
    bool find_start = true;
    //check if i is incremented or not
    if (caseL) {
        f(i, 0, t) {
            if (arr[i] == l && find_start) {
                end = i;
                find_start = false;
            }
            if (arr[i] == l) {
